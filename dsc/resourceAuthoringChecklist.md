...
La struttura di directory per la risorsa xRemoteFile, che fa parte del [modulo della risorsa xPSDesiredStateConfiguration](https://github.com/PowerShell/xPSDesiredStateConfiguration), potrebbe avere questo aspetto:
...
Verificare il file dello schema della risorsa (*. schema.mof). È possibile usare [Progettazione risorse DSC](https://www.powershellgallery.com/packages/xDSCResourceDesigner/) per sviluppare e testare lo schema.
Verificare che:
...
Ad esempio, nella configurazione precedente non è così evidente che il modo migliore per specificare UserAgent è:
