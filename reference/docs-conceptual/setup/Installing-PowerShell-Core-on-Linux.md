...
Scegliere quindi la soluzione `break powershell-6.1.0-1.rhel.7.x86_64 by ignoring some of its dependencies` quando si installa il pacchetto PowerShell.
...
Scaricare il pacchetto RPM `powershell-6.1.0-1.rhel.7.x86_64.rpm` dalla pagina delle [versioni][] nel computer OpenSUSE.
...
> [!NOTE]
> Il supporto di Kali attualmente non funziona. Usare il [pacchetto Snap][snap].
