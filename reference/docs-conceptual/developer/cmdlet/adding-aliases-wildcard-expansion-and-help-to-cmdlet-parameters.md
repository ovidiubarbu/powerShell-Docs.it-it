...
Il cmdlet deve definire i parametri che supportano le modifiche del sistema e il feedback dell'utente. Il cmdlet deve definire un parametro `Name` o un valore equivalente, in modo che il cmdlet possa modificare il sistema con un tipo di identificatore. Inoltre, il cmdlet deve definire i parametri `Force` e `PassThru`. Per ulteriori informazioni su questi parametri, vedere [creazione di un cmdlet che modifichi il sistema](./creating-a-cmdlet-that-modifies-the-system.md).
...
Nel codice seguente viene illustrato come impostare le opzioni relative ai caratteri jolly e definire il modello con caratteri jolly utilizzato per risolvere il parametro `Name` per questo cmdlet.
...
- A questo punto, effettuare la seguente voce per arrestare tutti i processi che corrispondono al modello con caratteri jolly "* Nota @ no__t-0". Viene richiesto prima di arrestare ogni processo che corrisponde al modello.
