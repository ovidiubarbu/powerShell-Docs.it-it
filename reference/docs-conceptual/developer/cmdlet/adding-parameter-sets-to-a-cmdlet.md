...
Quando il cmdlet è stato registrato con Windows PowerShell, testarlo eseguendolo nella riga di comando. Ecco alcuni test che illustrano come i parametri `ProcessId` e `InputObject` possono essere usati per testare i set di parametri per arrestare un processo.
