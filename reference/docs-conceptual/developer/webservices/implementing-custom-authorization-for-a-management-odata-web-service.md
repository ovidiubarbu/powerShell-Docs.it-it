...
L'uso del servizio Web Windows PowerShell richiede una terza parte per implementare l'interfaccia [Microsoft. Management. OData. CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) per esporre i cmdlet di Windows PowerShell. Questa interfaccia esegue l'autorizzazione utente per il servizio Web. Dopo aver scritto il codice per implementare l'interfaccia, è necessario compilarlo in una DLL da utilizzare nell'applicazione Web.
...
Nell'esempio seguente viene creata una classe che analizza i criteri di autorizzazione nel file XML.
...
```
 ...
 Le classi seguenti rappresentano rispettivamente i gruppi e gli utenti.
