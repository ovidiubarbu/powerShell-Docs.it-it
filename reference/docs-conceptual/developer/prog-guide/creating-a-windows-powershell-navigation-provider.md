...
L'implementazione predefinita accetta i percorsi con "/" o "\\" come separatore di percorso, normalizza il separatore di percorso su "\\", combina le parti del percorso padre e figlio con il separatore tra di essi, quindi restituisce una stringa che contiene i percorsi combinati.
...
2. Eseguire ora il cmdlet `Get-Childitem` per recuperare un elenco degli elementi del database, che sono le tabelle di database disponibili. Per ogni tabella, questo cmdlet recupera anche il numero di righe della tabella.
...
3. Utilizzare nuovamente il cmdlet `Set-Location` per impostare il percorso della tabella dati Employees.
...
7. Utilizzare nuovamente il cmdlet `Get-Item` per recuperare i dati dei dipendenti per gli elementi nella riga 0.
