...
---
# <a name="set-dsclocalconfigurationmanager-cmdlet-supports--force-parameter"></a><span data-ttu-id="77697-102">Il cmdlet Set-DscLocalConfigurationManager supporta il parametro -Force</span><span class="sxs-lookup"><span data-stu-id="77697-102">Set-DscLocalConfigurationManager cmdlet supports -force parameter</span></span>

<span data-ttu-id="77697-103">È stato aggiunto il supporto per il nuovo parametro al cmdlet Set-DscLocalConfigurationManager.</span><span class="sxs-lookup"><span data-stu-id="77697-103">We have added a support for new parameter to Set-DscLocalConfigurationManager cmdlet.</span></span> <span data-ttu-id="77697-104">Ciò consente all'utente di reimpostare la metaconfigurazione nel computer in modo deterministico quando sono in esecuzione in background altre operazioni come il controllo della coerenza, perché ciò causerà l'arresto di tutte le operazioni in esecuzione.</span><span class="sxs-lookup"><span data-stu-id="77697-104">This will allow the user to reset meta configuration on machine deterministically when other operations like consistency check are running in background as it will cause all running operations to be stopped.</span></span>

<span data-ttu-id="77697-105">L'esperienza è simile a questa quando si tenta di impostare la metaconfigurazione senza il parametro -Force.</span><span class="sxs-lookup"><span data-stu-id="77697-105">The experience looks like this when trying to set meta configuration without –Force parameter.</span></span>
```powershell
