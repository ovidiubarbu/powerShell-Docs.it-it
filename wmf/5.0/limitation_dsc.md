...
**Soluzione:** Creare nuovamente il certificato con utilizzo Data Encipherment o chiave di crittografia della chiave e dell'utilizzo chiavi avanzato crittografia documento (1.3.6.1.4.1.311.80.1). Nell'articolo Technet <https://technet.microsoft.com/library/dn807171.aspx> sono disponibili altre informazioni.
...
------------------------------------------------------
**Soluzione:** Eseguire il comando seguente in una sessione di PowerShell con privilegi elevata (eseguita come amministratore):
```powershell
...
**Soluzione:** Non premere CTRL + C durante il debug cmdlet Get-DscConfiguration.
...
**Soluzione:** Terminare il debug dell'operazione avviata da Get-DscConfiguration, come illustrato nella sezione '[risorse di debug DSC](https://msdn.microsoft.com/powershell/dsc/debugresource)'.
...
**Soluzione:** Individuare e associare allo spazio di esecuzione usando i cmdlet **Get-PSHostProcessInfo**, **Enter-PSHostProcess** , **Get-Runspace** e **Debug-Runspace** per eseguire il debug della risorsa DSC.
...
**Soluzione:** Usare nomi diversi per anche stesse risorse in configurazioni parziali diverse.
...
**Soluzione:** Sessione CIM da usare per operazioni DSC remote:
```powershell
...
**Soluzione:** Definire tutte le variabili e funzioni nella classe della risorsa DSC. Nessuna variabile/funzione con ambito $script.
...
**Soluzione:** Usare proprietà Credential, se disponibile. WindowsFeatureSet e ServiceSet di esempio
...
**Soluzione:** Importare la versione richiesta definendo il *ModuleSpecification* dell'oggetto per il `-ModuleName` con `RequiredVersion` chiave specificata nel modo seguente:
``` PowerShell
...
**Soluzione 2:** Modificare la configurazione DSC per pulire il *CommandAnalysis* cartella al termine della configurazione.
``` PowerShell
