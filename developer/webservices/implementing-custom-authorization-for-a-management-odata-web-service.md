---
title: Implementare l'autorizzazione personalizzata per un servizio web OData di gestione | Microsoft Docs
ms.custom: ''
ms.date: 09/13/2016
ms.reviewer: ''
ms.suite: ''
...
ms.locfileid: "62080657"
---
# <a name="implementing-custom-authorization-for-a-management-odata-web-service"></a><span data-ttu-id="f3381-102">Implementazione dell'autorizzazione personalizzata per un servizio Web OData di gestione</span><span class="sxs-lookup"><span data-stu-id="f3381-102">Implementing Custom Authorization for a Management OData web service</span></span>

<span data-ttu-id="f3381-103">Usando il servizio Web di Windows PowerShell richiede una terza parte implementare il [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interfaccia da esporre i cmdlet di Windows PowerShell.</span><span class="sxs-lookup"><span data-stu-id="f3381-103">Using the Windows PowerShell Web Service requires a third party to implement the [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface to expose Windows PowerShell cmdlets.</span></span> <span data-ttu-id="f3381-104">Questa interfaccia esegue l'autorizzazione utente per il servizio web.</span><span class="sxs-lookup"><span data-stu-id="f3381-104">This interface performs user authorization to the web service.</span></span> <span data-ttu-id="f3381-105">Dopo aver scritto il codice per implementare l'interfaccia, è necessario compilarlo in una DLL da utilizzare nell'applicazione web.</span><span class="sxs-lookup"><span data-stu-id="f3381-105">After writing the code to implement the interface, you must compile it into a DLL to be used in the web application.</span></span>

## <a name="pass-through-authorization"></a><span data-ttu-id="f3381-106">Autorizzazione di pass-through</span><span class="sxs-lookup"><span data-stu-id="f3381-106">Pass-through authorization</span></span>

<span data-ttu-id="f3381-107">Il modo più semplice per implementare il [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interfaccia è un'implementazione pass-through che autorizza tutti gli utenti.</span><span class="sxs-lookup"><span data-stu-id="f3381-107">The simplest way to implement the [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface is a pass-through implementation that authorizes all users.</span></span> <span data-ttu-id="f3381-108">Questo esempio viene non fornito alcun sicurezza e s fornito solo come un'illustrazione di come implementare l'interfaccia.</span><span class="sxs-lookup"><span data-stu-id="f3381-108">This example provides no security, and s provided only as an illustration of how to implement the interface.</span></span> <span data-ttu-id="f3381-109">Un'implementazione del [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interfaccia deve eseguire l'override di due metodi: [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) e [Microsoft.Management.Odata.CustomAuthorization.GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId).</span><span class="sxs-lookup"><span data-stu-id="f3381-109">An implementation of the  [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface must override two methods: [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) and [Microsoft.Management.Odata.CustomAuthorization.GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId).</span></span> <span data-ttu-id="f3381-110">In questo esempio, il [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) restituisce sempre il **System.Security.Principal.WindowsIdentity** oggetto associato all'utente corrente.</span><span class="sxs-lookup"><span data-stu-id="f3381-110">In this example, the [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) always returns the **System.Security.Principal.WindowsIdentity** object associated with the current user.</span></span>
...
### <a name="role-based-authorization"></a><span data-ttu-id="f3381-111">Autorizzazione basata sui ruoli</span><span class="sxs-lookup"><span data-stu-id="f3381-111">Role-based authorization</span></span>

<span data-ttu-id="f3381-112">L'esempio seguente implementa un criterio di autorizzazione basata sui ruoli.</span><span class="sxs-lookup"><span data-stu-id="f3381-112">The following example implements a role-based authorization policy.</span></span> <span data-ttu-id="f3381-113">I criteri vengono definiti in un file XML che si trova nella directory principale dell'applicazione con Web. config e i file MOF e XML schema di mapping.</span><span class="sxs-lookup"><span data-stu-id="f3381-113">The policy is defined in an XML file that resides in the main application directory with the web.config and MOF and XML mapping schema files.</span></span> <span data-ttu-id="f3381-114">Per informazioni su come configurare il file di schema di autorizzazione, vedere [autorizzazione basata sui ruoli di configurazione](./configuring-role-based-authorization.md).</span><span class="sxs-lookup"><span data-stu-id="f3381-114">For information about how to configure the authorization schema file, see [Configuring Role-based Authorization](./configuring-role-based-authorization.md).</span></span> <span data-ttu-id="f3381-115">La prima parte dell'esempio implementa il [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) e [Microsoft.Management.Odata.CustomAuthorization.GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId) metodi.</span><span class="sxs-lookup"><span data-stu-id="f3381-115">The first part of the sample implements the [Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.AuthorizeUser) and [Microsoft.Management.Odata.CustomAuthorization.GetMembershipId](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization.GetMembershipId) methods.</span></span> <span data-ttu-id="f3381-116">In questo caso, i metodi di interfaccia chiamano metodi `RbacSystem` classe (definito di seguito) che le effettive operazioni di verifica delle autorizzazioni per l'utente.</span><span class="sxs-lookup"><span data-stu-id="f3381-116">In this case, the interface methods call methods in the `RbacSystem` class (defined below) that do the actual work of checking the permissions for the user.</span></span>
...
<span data-ttu-id="f3381-117">L'esempio seguente crea una classe che analizza i criteri di autorizzazione nel file XML.</span><span class="sxs-lookup"><span data-stu-id="f3381-117">The following example creates a class that parses authorization policy in the XML file.</span></span>
...
```
 ...
 <span data-ttu-id="f3381-118">Le classi seguenti rappresentano rispettivamente i gruppi e utenti.</span><span class="sxs-lookup"><span data-stu-id="f3381-118">The following classes represent groups and users, respectively.</span></span>
...
<span data-ttu-id="f3381-119">Infine, la classe RbacSystem implementa metodi che l'operazione di verifica delle autorizzazioni per l'utente e restituisce lo stato di autorizzazione per i metodi definiti nell'implementazione del [Microsoft.Management.Odata.CustomAuthorization ](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interfaccia.</span><span class="sxs-lookup"><span data-stu-id="f3381-119">Finally, the RbacSystem class implements methods that do the work of checking the permissions for the user and return the authorization status to the methods defined in the implementation of the [Microsoft.Management.Odata.CustomAuthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) interface.</span></span>