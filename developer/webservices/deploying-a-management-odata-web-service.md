...
Dopo aver completato tutti i passaggi necessari per creare un servizio web OData di gestione, è necessario distribuirlo come un'applicazione web in IIS.

## <a name="deploying-the-web-service"></a>Distribuzione del servizio web

Completare la procedura seguente per distribuire il servizio web OData di gestione.

1. Creare una directory per la web dell'applicazione con il server IIS `WWWRoot` directory.

2. Copiare il file MOF file di schema, il file di schema XML, le DLL che esporta le [Microsoft.Management.Odata.Customauthorization](/dotnet/api/Microsoft.Management.Odata.CustomAuthorization) e [pssessionconfiguration](/dotnet/api/System.Management.Automation.Remoting.PSSessionConfiguration) interfacce e il file Web. config nella directory dell'applicazione.

3. Creare un ID del sito.

4. Creare e configurare un pool di applicazioni.

5. Configurare l'autenticazione per il sito.

6. Configurare il firewall.

7. Avviare il sito.

Gli script di Windows PowerShell seguenti viene illustrato come distribuire un servizio web OData di gestione.
