...
In questo esempio viene illustrata una versione completa del cmdlet Get-Process.

## <a name="how-to-build-the-sample-using-visual-studio"></a>Come compilare l'esempio utilizzando Visual Studio.

1. Aprire Windows Explorer e passare alla directory GetProcessSample05 nella directory degli esempi.

   Con Windows PowerShell 2.0 SDK installato, passare alla cartella GetProcessSample05. Il percorso predefinito è C:\Program Files (x86) \Microsoft SDKs\Windows\v7.0\Samples\sysmgmt\WindowsPowerShell\csharp\GetProcessSample05.
...
3. Nel **compilare** dal menu **Compila soluzione**.
   ...
   La libreria per il codice di esempio verrà compilata nella directory predefinita \bin o \bin\Debug.
...
- Specifica le posizioni per i parametri.

- Specifica che i parametri possono accettare l'input dalla pipeline. L'input può essere eseguita da un oggetto o un valore da una proprietà di un oggetto il cui nome di proprietà è identico al nome del parametro.
...
In questo esempio viene illustrato come creare un cmdlet che consente di visualizzare un elenco di processi specificati.
