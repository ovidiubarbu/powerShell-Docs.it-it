...
In questo esempio viene illustrato come supportare i processi quando si scrivono i cmdlet. Se si desidera che gli utenti di eseguire il cmdlet come processo in background, è necessario includere il codice descritto nella procedura seguente. Per altre informazioni sui processi in background, vedere [i processi in Background](./background-jobs.md).

## <a name="to-support-jobs"></a>Per supportare i processi

1. Definire un `AsJob` parametro opzionale in modo che l'utente può decidere se eseguire il cmdlet come processo.

    Nell'esempio seguente mostra una dichiarazione di parametro AsJob.
...
3. In un metodo di elaborazione record, aggiungere un `if` istruzione per rilevare se il cmdlet deve essere eseguito come processo. Il codice seguente usa il [System.Management.Automation.Cmdlet.ProcessRecord](/dotnet/api/System.Management.Automation.Cmdlet.ProcessRecord) (metodo).
...
4. Per gli oggetti di processo personalizzato, implementare la classe di processo.
...
5. Se il cmdlet esegue le operazioni, chiamare il [System.Management.Automation.Cmdlet.WriteObject](/dotnet/api/System.Management.Automation.Cmdlet.WriteObject) metodo per restituire un oggetto processo per la pipeline. Se il lavoro viene eseguito come processo, aggiungere processo figlio per il processo.
...
Esempio di codice seguente illustra il codice per un **Get-Proc** cmdlet che può recuperare i processi internamente o con un processo in background.
